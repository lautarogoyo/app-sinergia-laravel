...
export default function RemoveGrupo() {
  ...
  const { mutate, isPending } = useDeleteGrupo(() => navigate("/personas"));
      ...
      <div className="p-8 bg-gray-100 w-full flex flex-col items-center">
        <div className="w-full max-w-xl bg-white shadow-2xl rounded-xl border border-gray-300 p-6">
          ...
          <button type="button" onClick={() => navigate("/personas")}
            className="bg-blue-600 hover:bg-blue-700 text-white text-lg font-bold py-2 px-4 rounded shadow">
            Volver
          </button>
        </div>
      </div>
    ...
    <div className="p-8 bg-gray-100 w-full flex flex-col items-center">
      <div className="w-full max-w-xl bg-white shadow-2xl rounded-xl border border-gray-300 p-6">
        ...
        <p className="mb-6 text-red-600 text-lg">
          ¿Está seguro que desea eliminar el grupo <strong>{grupo.denominacion}</strong>?
          Esta acción no se puede deshacer.
        </p>
        <div className="flex gap-2">
          <button type="button" onClick={() => mutate(id)} disabled={isPending}
            className="bg-red-600 hover:bg-red-700 text-white text-lg font-bold py-2 px-4 rounded shadow disabled:opacity-50">
